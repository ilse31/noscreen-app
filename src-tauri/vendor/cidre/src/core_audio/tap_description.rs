...
impl TapDesc {
    ...
    // PATCHED (noscreen): `TapDesc` is a weakly-linked, version-gated class
    // (macOS 14.2+ Process Tap API), so `Self::alloc()` returns
    // `Option<arc::A<Self>>` here. Upstream's `with_*` convenience wrappers
    // didn't account for that and failed to compile under this crate's own
    // default feature set. We make them panic with a clear message instead
    // of silently miscompiling; callers who need a graceful fallback should
    // call `Self::alloc()` + the `init_*` method directly (as noscreen's own
    // `copilot::audio::start_loopback` does).
    pub fn with_stereo_mixdown_of_processes(
        processes_obj_ids_to_include: &ns::Array<ns::Number>,
    ) -> arc::R<Self> {
        Self::alloc()
            .expect("TapDesc unavailable (requires macOS 14.2+)")
            .init_stereo_mixdown_of_processes(processes_obj_ids_to_include)
    }
    ...
    pub fn with_stereo_global_tap_excluding_processes(
        ...
        Self::alloc()
            .expect("TapDesc unavailable (requires macOS 14.2+)")
            .init_stereo_global_tap_but_exclude_processes(processes_obj_ids_to_exclude)
    }
    ...
    pub fn with_mono_mixdown_of_processes(
        ...
        Self::alloc()
            .expect("TapDesc unavailable (requires macOS 14.2+)")
            .init_mono_mixdown_of_processes(processes_obj_ids_to_include)
    }
    ...
    pub fn with_mono_global_tap_excluding_processes(
        ...
        Self::alloc()
            .expect("TapDesc unavailable (requires macOS 14.2+)")
            .init_mono_global_tap_but_exclude_processes(processes_obj_ids_to_exclude)
    }
    ...
    pub fn with_processes_and_device(
        ...
        Self::alloc()
            .expect("TapDesc unavailable (requires macOS 14.2+)")
            .init_with_processes_and_device(
